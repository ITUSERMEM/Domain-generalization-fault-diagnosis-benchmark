...
def train(model):
    ...
    for i in range(1, iteration + 1):
        ...
        src_pred,domain_pred= model(src_data)

        cls_loss = F.nll_loss(F.log_softmax(src_pred, dim=1), src_label)

        loss = nn.CrossEntropyLoss()
        confusion_loss_total = loss(domain_pred, domain_label)

        lambd = 2 / (1 + math.exp(-10 * (i) / iteration)) - 1


        loss = cls_loss-confusion_loss_total*lambd
        ...
        if i % log_interval == 0:
            print('Train iter: {} [({:.0f}%)]\tLoss: {:.6f}\tsoft_Loss: {:.6f}\tconfu_Loss: {:.6f}'.format(
                i, 100. * i / iteration, loss.item(), cls_loss.item(),confusion_loss_total))
...
def test_source(model,test_loader):
    ...
    print('\n{} set: Average loss: {:.4f}, Accuracy: {}/{} ({:.2f}%)\n'.format(tgt_name,test_loss, correct, len(test_loader.dataset),100. * correct / len(test_loader.dataset)))
...
if __name__ == '__main__':
    ...
    Tasksetting = [CWRUTasksetting, PUTasksetting]



    for tasknumber in range(2):
        ...
        for taskindex in range(4):
            ...
            for repeat in range(10):
                ...
                root_path = '/home/Domain-generalization-fault-diagnosis-benchmark-main/' + dataset + 'data' + str(class_num) + '.mat'
                ...
                model = models.DeepALL_ADV(num_classes=class_num)
